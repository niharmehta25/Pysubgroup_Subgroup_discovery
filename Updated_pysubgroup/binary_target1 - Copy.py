...
class BinaryTarget:
    ...
    statistic_types = ('size_sg', 'size_dataset', 'positives_sg', 'positives_dataset', 'size_complement',
                      ...
                      'target_share_sg', 'target_share_complement', 'target_share_dataset', 'lift','arl_score')
    ...
    def all_statistics_present(self, cached_statistics):
        """
        Check if all required statistics are present in the cached statistics.
        This method should return True if all required statistics are present,
        and False otherwise.
        """
        required_statistics = ['size_sg', 'positives_sg', 'size_dataset', 'positives_dataset']
        if cached_statistics is None:
            return False

        return all(stat in cached_statistics for stat in required_statistics)
    ...
    def calculate_statistics(self, subgroup, data, cached_statistics=None):
        ...
        cover_arr, size_sg = ps.get_cover_array_and_size(subgroup, data_len=size_dataset, data=data)
        y_prob = cover_arr['y_prob']
        sorted_indices = np.argsort(cover_arr['y'])
        ...
        statastics['arl_score'] = arl_score
...
class ARLQualityFunction(ps.AbstractInterestingnessMeasure):
    tpl = namedtuple('ARL_parameters', ('average_ranking_loss',))
    ...
    def calculate_constant_statistics(self, data, target):
        assert isinstance(target, BinaryTarget)
        self.positives = target.covers(data)
        self.y_prob = data['y_prob'].values
        self.has_constant_statistics = True
    ...
    def calculate_statistics(self, subgroup, target, data, statistics=None):
        ...
        cover_arr, size_sg = ps.get_cover_array_and_size(subgroup, len(self.positives), data)
        subgroup_positives = self.positives[cover_arr]
        subgroup_probabilities = self.gp_get_stats(cover_arr)
        average_ranking_loss = self.arl(subgroup_positives, subgroup_probabilities)

        return ARLQualityFunction.tpl(average_ranking_loss)
    ...
    def calculate_statistics(self, subgroup, target, data, statistics=None):
        ...
        cover_arr, size_sg = ps.get_cover_array_and_size(subgroup, len(self.positives), data)
        subgroup_positives = self.positives[cover_arr]
        average_ranking_loss = self.arl(self.positives, self.y_prob, subgroup_positives, self.y_prob[cover_arr])

        return ARLQualityFunction.tpl(average_ranking_loss)
    
    def evaluate(self, subgroup, target, data, statistics=None):
        statistics = self.ensure_statistics(subgroup, target, data, statistics)
        return statistics.average_ranking_loss
    def arl(self, y, y_prob, subgroup_positives, subgroup_probabilities):
    ...
    # The following methods are for compatibility with the GpGrowth algorithm
    def gp_get_stats(self, row_index):
        return self.positives[row_index]

    def gp_get_null_vector(self):
        return np.zeros_like(self.positives, dtype=int)

    def gp_merge(self, l, r):
        l += r

    def gp_get_params(self, _cover_arr, v):
        return ARLQualityFunction.tpl(v[0])

    def gp_to_str(self, stats):
        return str(stats.average_ranking_loss)

    def gp_size_sg(self, stats):
        return stats.average_ranking_loss
