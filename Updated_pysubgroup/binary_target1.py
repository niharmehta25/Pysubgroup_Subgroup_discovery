...
class BinaryTarget:
    ...
    def calculate_statistics(self, subgroup, data, cached_statistics=None):
        ...
        cover_arr, size_sg = ps.get_cover_array_and_size(subgroup,  data)
        y_prob = np.array(cover_arr[:-1])
        y = np.array(cover_arr[:-2])
...
#
        arl_score= len(y)
        statistics['arl_score'] = arl_score
...
class ARLQualityFunction(ps.AbstractInterestingnessMeasure):
    ...
    def calculate_statistics(self, subgroup, target, data, statistics=None):
        ...
        arl_score = self.arl(self.positives, self.y_prob, subgroup_positives, self.y_prob[cover_arr])

        return ARLQualityFunction.tpl(average_ranking_loss)
    
    def evaluate(self, subgroup, target, data, statistics=None):
        if statistics is None:
            return 0.0  # Return a default value if statistics is None
        return getattr(statistics, 'arl_score', 0.0)
