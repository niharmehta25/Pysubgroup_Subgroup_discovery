...
class ARLQF(AbstractInterestingnessMeasure):
    ...
    from thesis.arl_function import calculate_arl_score
...
class ARL_Compare(AbstractInterestingnessMeasure):
    ...
    def calculate_statistics(self, subgroup, target, data, statistics=None):    
        ...
        y = np.array(target.covers(data)[cover_arr])
        y_prob = np.array(data['y_prob'][cover_arr])
        y_overall = np.array(target.covers(data))
        y_prob_overall = np.array(data['y_prob'])
        a = self.a
        b = ((len(y)/len(data['y_prob']))**a)
        print(len(y))
        print(len(data['y_prob']))
        arl_score = self.calculate_arl_score(y, y_prob,b)
        arl_score_overall = self.calculate_arl_score(y_overall, y_prob_overall,b)
        ARL_ratio = arl_score_overall/arl_score
        return ARL_ratio
...
class SubgroupModelAccuracy:
    ...
    def train_and_evaluate_model(self, subgroup, model_name, data, b):
        cover_arr, _ = ps.get_cover_array_and_size(subgroup, len(data), data)
        ...
        if unique_classes < 2:
            # Skip Logistic Regression model training if there's only one class
            return 0.0
    
       
        model = clone(model_name)
        
        cv = KFold(n_splits=5, shuffle=True, random_state=42)
        scores = cross_val_score(model, X, y, cv=cv, scoring='accuracy')
        mean_accuracy = scores.mean()
    
        accuracy = (1 - mean_accuracy) * b
        return accuracy
    ...
    def calculate_statistics(self, subgroup, target,model_name, data, statistics=None):
        if self.model_name is None:
            raise ValueError(" model_name and must be provided.")
        cover_arr, _ = ps.get_cover_array_and_size(subgroup, len(data), data)
        y = np.array(target.covers(data)[cover_arr])
        y_prob = np.array(data['y_prob'][cover_arr])
        a = self.a
        b = ((len(y)/len(data['y_prob']))**a)
        accuracy = self.train_and_evaluate_model(subgroup, target,model_name, data,b)
        return accuracy
    ...
    def evaluate(self, subgroup, target,model_name,data, statistics=None):
